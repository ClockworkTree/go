...
func (gcToolchain) gc(b *Builder, a *Action, archive string, importcfg []byte, symabis string, asmhdr bool, gofiles []string) (ofile string, output []byte, err error) {
	...
	if p.Standard && (p.ImportPath == "std/internal/cpu" || p.ImportPath == "std/internal/bytealg") {
	...
	if p.Standard {
		switch p.ImportPath {
		case "bytes", "std/internal/poll", "net", "os", "runtime/pprof", "runtime/trace", "sync", "syscall", "time":
			extFiles++
		}
	}
	if extFiles == 0 {
